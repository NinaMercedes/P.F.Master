...
def main_map(args):
    ...
    if getattr(args, "trimmed", False):
        if args.single:
            args.reads = f"{args.prefix}_trimmed.fq"
        else:
            args.reads = f"{args.prefix}_1P {args.prefix}_2P"
    else:
        # No trimming; use original input reads
        if args.single:
            args.reads = args.read1
        else:
            args.reads = f"{args.read1} {args.read2}"
    ...
    if getattr(args, "trimmed", False) and args.single:
        fm.run_cmd(f"rm -f {args.reads}")
    if getattr(args, "trimmed", False) and not args.single:
        fm.run_cmd(f"rm -f {args.prefix}_1P {args.prefix}_2P {args.prefix}_1U {args.prefix}_2U")
...
parser_sub.add_argument('--trim', action='store_true', help='Run trimming step using Trimmomatic before mapping')
